...
# ── LOGO (øvre venstre) ───────────────────────────────────────────────────────
LX = 60
logo = Image.open(os.path.join(ROOT, "logo/exday_logo_inverse.png")).convert("RGBA")
logo_w = 210
...
draw.text((LX, 302), "Yield, ex-dato, score og kalkulator for alle norske", font=fnt(17, bold=False), fill=(134, 239, 172))
...
# ── TELEFON-MOCKUP ────────────────────────────────────────────────────────────
PX, PY, PW, PH, PR = 820, 40, 260, 540, 36
...
for ticker, name, yld, ex, score, score_col, highlight in aksjer:
    bg = (20, 83, 45) if highlight else CARD
    draw.rounded_rectangle([PX+10, AY, PX+PW-10, AY+52], radius=8, fill=bg)
...
tx = PX + 22
for i, (icon, label) in enumerate([("📊","Oversikt"),("📅","Kalender"),("💼","Portefølje"),("🧮","Kalk.")]):
